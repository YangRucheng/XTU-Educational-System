...
作者: WindrunnerMax
[原项目GitHub仓库](https://github.com/WindrunnerMax/SWVerifyCode)
...
with open(os.path.join(os.path.dirname(__file__), 'CharMap.json'), 'r', encoding='utf-8') as fr:
    charMap = json.load(fr)
...
def main(img: bytes) -> str:
    """ 识别验证码, 参数为图片, 返回识别结果 """
    img = Convert().run(img)
    # cv2.imwrite("img.jpg", img)  # 写入处理后的图片

    # 切割的位置
    im_position = ([7, 7, 7, 7], [[5, 12], [15, 22], [25, 32], [34, 41]], [
                   [4, 15], [4, 15], [4, 15], [4, 15]])
